...
async fn create_todo(db_pool: Arc<Pool<Sqlite>>) -> i32 {
...
async fn test_list_action() {
    ...
    let _ = create_todo(db_pool.clone()).await;
...
async fn test_add_action() {
    ...
    let last_todo_from_db: Todo = sqlx::query_as("SELECT * FROM todo ORDER BY ROWID DESC LIMIT 1")
        .fetch_one(&*db_pool)
        .await
        .unwrap();
...
async fn test_done_action() {
    ...
    let c_todo = create_todo(db_pool.clone()).await;

    // run done_todo for created todo
    TodoOptions::done_todo(db_pool.clone(), &[c_todo.to_string()])
        .await
        .unwrap();
...
async fn test_delete_todo() {
    ...
    TodoOptions::delete_todo(db_pool.clone(), &[c_todo.to_string()])
        .await
        .unwrap();
